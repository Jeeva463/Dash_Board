...
public interface OverTimeRepository extends JpaRepository<OverTime, Integer> {
    ...
    @Query(value = " select sum(oa.overtime_hours) from overtime_analysis oa where (oa.project_name=:value or oa.phase_name =:value or oa.job_name = :value) and oa.day=:dayType and oa.overtime_hours > 0 and To_DATE(oa.attendance_date, 'dd/MM/yyyy') BETWEEN To_DATE(:fromDate,'dd/MM/yyyy') AND To_DATE(:toDate,'dd/MM/yyyy') ", nativeQuery = true)
